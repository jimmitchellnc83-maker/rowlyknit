...
const PAGE_SIZE = 50;
// Safety cap — avoids a runaway loop if Ravelry returns a bad paginator.
const MAX_PAGES = 30;

export default function RavelryStashSync() {
  ...
  const handleImport = async () => {
    ...
    for (let page = 1; page <= MAX_PAGES; page++) {
      ...
      try {
        ...
        const message = err.response?.data?.message || err.message || 'Stash import failed';
        setErrorMessage(`Page ${page}: ${message}`);
        setPhase('error');
        toast.error(`Stash import failed on page ${page}: ${message}`);
        return;
      }
    }
    ...
    if (importedRunning > 0) {
      toast.success(
        `Imported ${importedRunning} yarn${importedRunning === 1 ? '' : 's'} from your Ravelry stash.`
      );
    } else {
      toast.info('Your Ravelry stash is already in Rowly — nothing new to import.');
    }
  };
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Bulk-import your Ravelry stash into your Rowly yarn list. Already-imported entries are
            skipped so your local edits stay intact.
          </p>
        </div>
        <button
      ...
      {!loading && oauthRequired && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-6">
            You'll need to link your Ravelry account before your stash can be imported.
          </p>
          <button
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-8">
          {phase === 'idle' && (
            <div className="text-center">
              <FiPackage className="mx-auto h-16 w-16 text-purple-500 mb-4" />
              <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">
                Ready to import
              </h3>
              <p className="text-gray-500 dark:text-gray-400 mb-6">
                Click below to fetch your Ravelry stash. This may take a few seconds for large
                stashes — you'll see live progress.
              </p>
              <button
          ...
          {phase === 'running' && (
            <div className="text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-2">
                ...
                {totalResults ? ` · ${totalResults} total in stash` : ''}
              </p>
          ...
          {phase === 'done' && (
            <div className="text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                {importedTotal} yarn{importedTotal === 1 ? '' : 's'} added to your stash
