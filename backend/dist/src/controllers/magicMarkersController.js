...
exports.getActiveMarkersForRow = getActiveMarkersForRow;
...
exports.snoozeMagicMarker = snoozeMagicMarker;
exports.completeMagicMarker = completeMagicMarker;
 ...
 */
function isMarkerRelevantForRow(marker, row) {
    // Check row range
    ...
    if (condition && Object.keys(condition).length > 0) {
        switch (marker.trigger_type) {
            case 'counter_value':
                if (condition.operator === 'equals' && row !== condition.value)
                    return false;
                if (condition.operator === 'greater_than' && row <= condition.value)
                    return false;
                if (condition.operator === 'less_than' && row >= condition.value)
                    return false;
                if (condition.operator === 'multiple_of' && row % condition.value !== 0)
                    return false;
                break;
            case 'row_interval':
                if (condition.interval && row % condition.interval !== 0)
                    return false;
                break;
        }
    }
...
async function getMagicMarkers(req, res) {
    ...
    let query = (0, database_1.default)('magic_markers').where({ project_id: projectId });
    if (counterId)
    ...
    const parsedMarkers = magicMarkers.map(marker => {
        ...
        if (currentRow) {
            parsed.isRelevantNow = isMarkerRelevantForRow(parsed, parseInt(currentRow, 10));
        }
        return parsed;
    });
 ...
 */
async function getActiveMarkersForRow(req, res) {
    const userId = req.user.userId;
    const { id: projectId } = req.params;
    const { row, counterId } = req.query;
    if (!row)
        throw new errorHandler_1.ValidationError('Row number is required');
    ...
    relevantMarkers.sort((a, b) => (priorityOrder[b.priority] || 2) -
...
async function createMagicMarker(req, res) {
    const userId = req.user.userId;
    const { id: projectId } = req.params;
    const { counterId, name, triggerType, triggerCondition, alertMessage, alertType, isActive = true, startRow, endRow, repeatInterval, repeatOffset, isRepeating, priority, displayStyle, color, category, } = req.body;
    ...
    if ((startRow !== undefined && Number.isNaN(parsedStartRow)) || (endRow !== undefined && Number.isNaN(parsedEndRow))) {
        throw new errorHandler_1.ValidationError('Start and end rows must be numeric');
    }
    if (!name || !alertMessage) {
        throw new errorHandler_1.ValidationError('Name and alert message are required');
    }
    ...
    if (priority && !validPriorities.includes(priority)) {
        throw new errorHandler_1.ValidationError(`Priority must be one of: ${validPriorities.join(', ')}`);
    }
    const validDisplayStyles = ['banner', 'popup', 'toast', 'inline'];
    if (displayStyle && !validDisplayStyles.includes(displayStyle)) {
        throw new errorHandler_1.ValidationError(`Display style must be one of: ${validDisplayStyles.join(', ')}`);
    }
    const validCategories = ['reminder', 'at_same_time', 'milestone', 'shaping', 'note'];
    if (category && !validCategories.includes(category)) {
        throw new errorHandler_1.ValidationError(`Category must be one of: ${validCategories.join(', ')}`);
    }
    ...
    if (triggerCondition) {
        try {
            ...
            throw new errorHandler_1.ValidationError('Trigger condition must be valid JSON');
        }
    }
    if (triggerType === 'counter_value') {
        ...
        if (!normalizedCondition.operator || !allowedOperators.includes(normalizedCondition.operator)) {
            throw new errorHandler_1.ValidationError(`Counter trigger requires operator: ${allowedOperators.join(', ')}`);
        }
        const numericValue = Number(normalizedCondition.value);
        if (normalizedCondition.value === undefined || Number.isNaN(numericValue)) {
            throw new errorHandler_1.ValidationError('Counter trigger requires a numeric value');
        }
    ...
    if (triggerType === 'row_interval') {
        ...
        if (!intervalValue || intervalValue <= 0) {
            throw new errorHandler_1.ValidationError('Row interval trigger requires a positive interval');
        }
        normalizedCondition.interval = intervalValue;
    }
    if (parsedStartRow !== undefined && parsedEndRow !== undefined && parsedStartRow > parsedEndRow) {
        throw new errorHandler_1.ValidationError('Start row must be less than or equal to end row');
    }
    if (isRepeating && (!parsedRepeatInterval || parsedRepeatInterval <= 0)) {
        throw new errorHandler_1.ValidationError('Repeat interval must be a positive number when repeating is enabled');
    }
    if (parsedRepeatOffset !== undefined && Number.isNaN(parsedRepeatOffset)) {
        throw new errorHandler_1.ValidationError('Repeat offset must be numeric');
    }
    const project = await (0, database_1.default)('projects')
        .where({ id: projectId, user_id: userId })
        .whereNull('deleted_at')
...
async function updateMagicMarker(req, res) {
    ...
    const parsedStartRow = updates.startRow !== undefined ? Number(updates.startRow) : magicMarker.start_row;
    const parsedEndRow = updates.endRow !== undefined ? Number(updates.endRow) : magicMarker.end_row;
    if (updates.startRow !== undefined) {
        if (Number.isNaN(parsedStartRow))
            throw new errorHandler_1.ValidationError('Start row must be numeric');
        updateData.start_row = parsedStartRow;
    }
    if (updates.endRow !== undefined) {
        if (Number.isNaN(parsedEndRow))
            throw new errorHandler_1.ValidationError('End row must be numeric');
        updateData.end_row = parsedEndRow;
    }
    if ((updates.startRow !== undefined || updates.endRow !== undefined) &&
        parsedStartRow !== null &&
        parsedEndRow !== null &&
        parsedStartRow > parsedEndRow) {
        throw new errorHandler_1.ValidationError('Start row must be less than or equal to end row');
    }
    const parsedRepeatInterval = updates.repeatInterval !== undefined
        ? Number(updates.repeatInterval)
        : magicMarker.repeat_interval;
    const parsedRepeatOffset = updates.repeatOffset !== undefined ? Number(updates.repeatOffset) : magicMarker.repeat_offset;
    ...
    if (updates.isRepeating !== undefined)
        updateData.is_repeating = updates.isRepeating;
    const validPriorities = ['low', 'normal', 'high', 'critical'];
 ...
 */
async function snoozeMagicMarker(req, res) {
    const userId = req.user.userId;
    const { id: projectId, markerId } = req.params;
    const { duration } = req.body;
    if (!duration || duration <= 0) {
        throw new errorHandler_1.ValidationError('Snooze duration must be a positive number of minutes');
    }
    const project = await (0, database_1.default)('projects')
        .where({ id: projectId, user_id: userId })
        .whereNull('deleted_at')
 ...
 */
async function completeMagicMarker(req, res) {
    const userId = req.user.userId;
    const { id: projectId, markerId } = req.params;
    const project = await (0, database_1.default)('projects')
        .where({ id: projectId, user_id: userId })
 ...
 */
async function recordTrigger(req, res) {
    const userId = req.user.userId;
    const { id: projectId, markerId } = req.params;
    const project = await (0, database_1.default)('projects')
        .where({ id: projectId, user_id: userId })
...
async function checkMagicMarkers(projectId, counterId, counterValue) {
    ...
    for (const marker of magicMarkers) {
        ...
        if (isMarkerRelevantForRow(parsed, counterValue)) {
            ...
            await (0, database_1.default)('magic_markers')
                .where({ id: marker.id })
                .update({
                last_triggered: new Date(),
    ...
    const priorityOrder = { critical: 4, high: 3, normal: 2, low: 1 };
    triggeredMarkers.sort((a, b) => (priorityOrder[b.priority] || 2) -
        (priorityOrder[a.priority] || 2));
