...
class PatternExportService {
  ...
  calculateYarnRequirements(
    baseYardage: number,
    lengthAdjustmentPercent: number = 0,
    widthAdjustmentPercent: number = 0
  ): YarnCalculation {
   ...
   */
  lengthToPercentage(lengthChange: number, unit: string = 'inches'): number {
   ...
   */
  widthToPercentage(widthChange: number, unit: string = 'inches'): number {
  ...
  async generateExportPDF(
    ...
    try {
      ...
      // Title
      addText(patternData.name, timesRomanBold, 24, rgb(0.1, 0.1, 0.3));
      yPosition -= 8;

      // Designer
      if (patternData.designer) {
        addText(`by ${patternData.designer}`, timesRoman, 14, rgb(0.4, 0.4, 0.4));
      }
      ...
      addText('Pattern Information', helveticaBold, 14, rgb(0.2, 0.2, 0.5));
      yPosition -= 4;

      if (patternData.category) {
        addText(`Category: ${patternData.category}`, helvetica, 11, rgb(0.3, 0.3, 0.3), 10);
      }
      if (patternData.difficulty) {
        addText(`Difficulty: ${patternData.difficulty}`, helvetica, 11, rgb(0.3, 0.3, 0.3), 10);
      }
      if (patternData.source_url) {
        addText(`Source: ${patternData.source_url}`, helvetica, 10, rgb(0.3, 0.3, 0.6), 10);
      }

      // Selected Size
      const selectedSize = options.selectedSize || projectData?.selected_size;
      if (selectedSize) {
        addText(`Selected Size: ${selectedSize}`, helvetica, 11, rgb(0.3, 0.3, 0.3), 10);
      }
      ...
      if (options.includeGauge && patternData.gauge) {
        addLine();
        yPosition -= 10;
        addText('Gauge', helveticaBold, 14, rgb(0.2, 0.2, 0.5));
        yPosition -= 4;
        ...
        if (gaugeText) {
          addText(gaugeText, helvetica, 11, rgb(0.3, 0.3, 0.3), 10);
        }
        yPosition -= 10;
      }
      ...
      if (patternData.needle_sizes && patternData.needle_sizes.length > 0) {
        addLine();
        yPosition -= 10;
        addText('Needle Sizes', helveticaBold, 14, rgb(0.2, 0.2, 0.5));
        yPosition -= 4;

        patternData.needle_sizes.forEach((needle: any) => {
          let needleText = '';
          if (needle.us) needleText += `US ${needle.us}`;
          if (needle.mm) needleText += ` (${needle.mm}mm)`;
          if (needle.type) needleText += ` - ${needle.type}`;
          if (needleText) {
            addText(`• ${needleText}`, helvetica, 11, rgb(0.3, 0.3, 0.3), 10);
          }
        });
        yPosition -= 10;
      }
      ...
      if (options.includeYarnRequirements) {
        addLine();
        yPosition -= 10;
        addText('Yarn Requirements', helveticaBold, 14, rgb(0.2, 0.2, 0.5));
        yPosition -= 4;
        ...
        if (patternData.yarn_requirements && patternData.yarn_requirements.length > 0) {
          patternData.yarn_requirements.forEach((yarn: any, index: number) => {
            const yarnName = yarn.name || yarn.weight || `Yarn ${index + 1}`;
            const baseYardage = yarn.yardage || patternData.estimated_yardage || 0;

            addText(`${yarnName}:`, helveticaBold, 11, rgb(0.2, 0.2, 0.2), 10);

            if (yarn.weight) {
              addText(`  Weight: ${yarn.weight}`, helvetica, 10, rgb(0.4, 0.4, 0.4), 20);
            }
            if (yarn.fiber) {
              addText(`  Fiber: ${yarn.fiber}`, helvetica, 10, rgb(0.4, 0.4, 0.4), 20);
            }
            if (yarn.color) {
              addText(`  Color: ${yarn.color}`, helvetica, 10, rgb(0.4, 0.4, 0.4), 20);
            }

            if (baseYardage > 0) {
              const calculation = this.calculateYarnRequirements(
                baseYardage,
                lengthAdjustmentPercent,
                widthAdjustmentPercent
              );

              if (calculation.percentageIncrease !== 0) {
                addText(`  Base Yardage: ${calculation.baseYardage} yards`, helvetica, 10, rgb(0.4, 0.4, 0.4), 20);
                addText(
                  `  Size Adjustment: ${calculation.percentageIncrease > 0 ? '+' : ''}${calculation.percentageIncrease.toFixed(1)}%`,
                  helvetica, 10, rgb(0.6, 0.4, 0.2), 20
                );
                addText(
                  `  Adjusted Yardage: ${calculation.adjustedYardage} yards`,
                  helveticaBold, 11, rgb(0.1, 0.5, 0.1), 20
                );
              } else {
                addText(`  Yardage: ${baseYardage} yards`, helvetica, 10, rgb(0.4, 0.4, 0.4), 20);
              }
            }
          ...
          const calculation = this.calculateYarnRequirements(
            patternData.estimated_yardage,
            lengthAdjustmentPercent,
            widthAdjustmentPercent
          );

          addText('Estimated Yarn:', helveticaBold, 11, rgb(0.2, 0.2, 0.2), 10);
          ...
          if (calculation.percentageIncrease !== 0) {
            ...
            addText(`  Estimated: ${patternData.estimated_yardage} yards`, helvetica, 10, rgb(0.4, 0.4, 0.4), 20);
          }
        }
        ...
        if (options.includeSizeAdjustments && (lengthAdjustmentPercent !== 0 || widthAdjustmentPercent !== 0)) {
          yPosition -= 8;
          addText('* Yarn adjustments calculated based on your size modifications', helvetica, 9, rgb(0.5, 0.5, 0.5), 10);
        }
      ...
      if (options.includeNotes && patternData.notes) {
        addLine();
        yPosition -= 10;
        addText('Notes', helveticaBold, 14, rgb(0.2, 0.2, 0.5));
        yPosition -= 4;

        // Wrap long notes
        const noteLines = this.wrapText(patternData.notes, 80);
        noteLines.slice(0, 20).forEach((line) => {
          addText(line, helvetica, 10, rgb(0.4, 0.4, 0.4), 10);
        });
        if (noteLines.length > 20) {
          addText('...', helvetica, 10, rgb(0.4, 0.4, 0.4), 10);
        }
        yPosition -= 10;
      }
      ...
      yPosition = 40;
      page.drawText(
        `Generated by RowlyKnit on ${new Date().toLocaleDateString()}`,
        {
          x: leftMargin,
          y: yPosition,
          size: 9,
      ...
      logger.info('Pattern exported to PDF', {
        userId,
        patternId,
        projectId,
        filename,
      ...
      logger.error('Error generating pattern export PDF', {
        userId,
        patternId,
        projectId,
        error: message,
...
export default new PatternExportService();
