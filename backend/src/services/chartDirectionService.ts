...
class ChartDirectionService {
   ...
   */
  calculateRowDirection(workingDirection: WorkingDirection, rowNumber: number): CurrentDirection {
   ...
   */
  getRowStartPosition(chartWidth: number, direction: CurrentDirection): number {
    switch (direction) {
      case 'left_to_right':
  ...
  async advanceStitch(
    ...
    const workingDirection = progress.working_direction as WorkingDirection;
    const currentDirection = progress.current_direction as CurrentDirection;

    let newPosition: StitchPosition;

    if (direction === 'forward') {
      newPosition = this.getNextStitchPosition(
      ...
      const cellExists = completedCells.some(
        (c: any) => c.row === progress.current_row && c.col === progress.current_column
      );
      if (!cellExists) {
      ...
      await db('chart_direction_history').insert({
      ...
      // Backward direction
      newPosition = this.getPreviousStitchPosition(
        progress.current_row,
        progress.current_column,
        chartWidth,
        currentDirection,
      ...
      completedCells = completedCells.filter(
        (c: any) => !(c.row === newPosition.row && c.col === newPosition.col)
      );
  ...
  async toggleDirection(
    ...
    const newDirection: CurrentDirection =
      currentDirection === 'left_to_right' ? 'right_to_left' : 'left_to_right';
...
export default new ChartDirectionService();
